...
## コマンド
### サーバー管理（PM2）
- **サーバー起動（PM2）**: `npm run pm2:start` ⭐推奨
- **サーバー状態確認**: `npm run pm2:status`
- **サーバー再起動**: `npm run pm2:restart`
- **サーバーログ確認**: `npm run pm2:logs`
- **サーバー停止**: `npm run pm2:stop`
- **手動起動（非推奨）**: `npm start`
...
#### PCから（同一ネットワーク）
- **AI完全生成版（ルート）**: `http://192.168.10.7:3000/` ⭐推奨（最新版が自動表示）
- **AI完全生成版（直接）**: `http://192.168.10.7:3000/ai-complete-creator.html`
- **改良版アプリ**: `http://192.168.10.7:3000/android-working-creator.html`
- **従来版アプリ**: `http://192.168.10.7:3000/gemini-suno-creator.html`

**📍 PCアクセス要件**:
- AndroidデバイスとPCが同じWi-Fiネットワークに接続
- AndroidデバイスIP: `192.168.10.7` （動的に検出）
- サーバーポート: `3000`
- CORS設定済み（外部アクセス許可）
- 動的IP対応（起動時に正しいIPを表示）
...
## サーバー永続化設定 (2025-09-04)
### PM2によるプロセス管理
- **自動復旧**: サーバーがクラッシュしても自動的に再起動
- **状態保存**: `pm2 save`でプロセス設定を保存
- **Termux起動時の自動実行**: `.bashrc`に起動スクリプトを設定

### Termux自動起動設定
```bash
# .bashrc に追加されたスクリプト
# Termuxを開くたびに自動的にサーバーをチェック・起動
```
